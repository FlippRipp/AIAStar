...
bool AFGGridActor::GetWorldLocationFromIndex(int Index, FVector& WorldLocation)
{
	const FIntVector XY = GetXYFromIndex(Index);
	FIntVector YX = GetXYFromIndex(Index);
	UE_LOG(LogTemp, Log, TEXT("fv %f"), YX.X)
	
	if(XY.X != -1)
	{
		WorldLocation = GetWorldLocationFromXY(XY.X, XY.Y);
		return  true;
	}
	return false;
}
...
FIntVector AFGGridActor::GetXYFromIndex(int32 Index)
{
...
bool AFGGridActor::IsTileIndexValid(int32 TileIndex, bool Debug) const
{
	if(Debug)
	{
		UE_LOG(LogTemp, Log, TEXT("Index in IsTileIndexValid = %f"), TileIndex);
	}
...
TArray<FFGTileinfo> AFGGridActor::GetTileNeighbours(int TileIndex)
{
	TArray<FFGTileinfo> TileNeighbours;

	if(!IsTileIndexValid(TileIndex, true)) return TileNeighbours;

	if(IsTileIndexValid(TileIndex + 1))
	{
		FFGTileinfo Neighbour1 = TileList[TileIndex + 1];
		if(!Neighbour1.bBlock)
		{
			Neighbour1.Index = TileIndex + 1;
			TileNeighbours.Add(Neighbour1);
		}
	}

	if(IsTileIndexValid(TileIndex - 1))
	{
		FFGTileinfo Neighbour2 = TileList[TileIndex - 1];
		if(!Neighbour2.bBlock)
		{
			Neighbour2.Index = TileIndex - 1;
			TileNeighbours.Add(Neighbour2);
		}

	}

	if(IsTileIndexValid(TileIndex + Width))
	{
		FFGTileinfo Neighbour3 = TileList[TileIndex + Width];
		if(!Neighbour3.bBlock)
		{
			Neighbour3.Index = TileIndex + Width;
			TileNeighbours.Add(Neighbour3);
		}
	}

	if(IsTileIndexValid(TileIndex - Width))
	{
		FFGTileinfo Neighbour4 = TileList[TileIndex - Width];
		if(!Neighbour4.bBlock)
		{
			Neighbour4.Index = TileIndex - Width;
			TileNeighbours.Add(Neighbour4);
		}
	}
